...
class Ant:
    ...
    def __init__(self):
        self.visited_nodes = []
        self.visited_nodes.append(random.randint(0, NODE_COUNT - 1))
    ...
    def get_distance_travelled(self):
        ...
        total_distance += node_distances[self.visited_nodes[0]][self.visited_nodes[len(self.visited_nodes) - 1]]############# usun
...
class ACO:
    ...
    def setup_ants(self, number_of_ants_factor):
        ...
        for i in range(0, number_of_ants):
            self.ant_colony.append(Ant())############################ return ant_colony ?????
    ...
    def move_ants(self, ant_population):
        for ant in ant_population:
            ant.visit_node(self.pheromone_trails)
    ...
    def update_pheromones(self, evaporation_rate):##------------------------------------------------------------------- Blad nr 2 Feromony caly czas sa te same
        ...
        for ant in self.ant_colony:
            for node in range(0, len(ant.visited_nodes)):
                for x in range(0, NODE_COUNT):
                    for y in range(0, NODE_COUNT):
                        if ant.visited_nodes[node] == x and ant.visited_nodes[(node + 1) % 5] == y:
    ...
    def solve(self, TOTAL_ITERATIONS, evaporation_rate):
        ...
        for i in range(0, TOTAL_ITERATIONS):
            self.setup_ants(NUMBER_OF_ANTS_FACTOR)  # Utworzenie populacji mrówek
